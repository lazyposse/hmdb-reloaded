...
(defn- get-fn-name "hack to get the name of a fn"
  [f] (second (s/split (clojure.main/demunge (str (getname f)))
                       #"[/@]")))

(defn- sorted-map-all "Recursively transforms nested maps into sorted nested maps. Usefull for comparing two nested datastructures"
  [m] (w/postwalk #(if (map? %)
                     (into (sorted-map) %)
                     %)
                  m))

(defn- loggify "log decorator"
  [f] (fn [& args]
