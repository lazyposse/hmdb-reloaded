...
** IN-PROGRESS add a way to inject the callback to the agent [0%]
*** IN-PROGRESS We need to put the Aspect code in a proper pkg

Because I don't know how to use default package classes in clojure

*** TODO verify that we can inject a clj code in the callback
