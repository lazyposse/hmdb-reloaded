...
*** IN-PROGRESS code a simple clojure callback that dump the calls in a atom [33%]

**** DONE design
CLOSED: [2012-08-26 Sun 19:50]

We take the same structure as the one used by clojure.xml to emit xml.
See =scratch.clj=
Advantage: we can export call stacks easily in xml.


**** IN-PROGRESS coding the capture in an atom [40%]
...
***** IN-PROGRESS serialize args/return values along with the method calls [100%]

****** DONE is it possible to merge clojure.xml with XStream xml? => yes! :) [100%]
