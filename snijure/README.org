...
**** IN-PROGRESS code the agent so that we weave only the given package(s) [100%]
...
CLOSED: [2012-08-17 Fri 16:56]

don't know how to do that, but there's a config file for that ! (aop.xml)
***** CANCELLED use the aop.xml file to specify which classes to weave
CLOSED: [2012-08-18 Sat 02:46]
